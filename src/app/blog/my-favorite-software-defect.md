...
allowed programmatic trading without much (any) fuss.
...
I just started that PC up to confirm the cpu model; it still works surprisingly well,
...
At first, things went smoothly, but eventually even a powerful cpu struggled to calculate
who-knows-how-many decimal places.

## the solution

The fix was to round the stored indicator value to two decimal places after each update.
...
Also, please note, this is old code; I will likely write more than one blog post about why
I dislike acronyms (and initialisms, hehe I'm a pedant).
I had to intentionally avoid saying the word "hate" there, not because I don't hate them,
but because I wanted to convey that I have a reasonable, logical basis for that disdain
...
problem.
By not writing that extra test to prove, without a doubt, that what you think is happening
...
Tests using unexpected data or randomized data or extreme data.

Ever since, I write software under the assumption that it will run indefinitely.
There's nothing quite like letting a piece of software run for a month to highlight that
shortcut you hoped would go unnoticed.
